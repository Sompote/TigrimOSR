...
impl P {
    ...
    fn scripts(&mut self, base: MathNode) -> MathNode {
        ...
        for _ in 0..2 {
            ...
            match self.peek() {
                Some('^') if sup.is_none() => {
                    self.adv();
                    sup = Some(Box::new(self.req()));
                }
                Some('_') if sub.is_none() => {
                    self.adv();
                    sub = Some(Box::new(self.req()));
                }
                _ => break,
            }
        }
    ...
    fn backslash(&mut self) -> Option<MathNode> {
        ...
        if self.peek() == Some('\\') {
            self.adv();
            return Some(MathNode::Space(0.5));
        }
        ...
        match cmd.as_str() {
            ...
            "dot" => {
                let a = self.req();
                Some(MathNode::Accent(Box::new(a), AccentKind::Dot))
            }
            "ddot" => {
                let a = self.req();
                Some(MathNode::Accent(Box::new(a), AccentKind::DDot))
            }
            "hat" | "widehat" => {
                let a = self.req();
                Some(MathNode::Accent(Box::new(a), AccentKind::Hat))
            }
            "bar" | "overline" => {
                let a = self.req();
                Some(MathNode::Accent(Box::new(a), AccentKind::Bar))
            }
            "vec" | "overrightarrow" => {
                let a = self.req();
                Some(MathNode::Accent(Box::new(a), AccentKind::Vec))
            }
            "tilde" | "widetilde" => {
                let a = self.req();
                Some(MathNode::Accent(Box::new(a), AccentKind::Tilde))
            }
            "check" => {
                let a = self.req();
                Some(MathNode::Accent(Box::new(a), AccentKind::Check))
            }
            // Boxed
            "boxed" => {
                let a = self.req();
                Some(MathNode::Boxed(Box::new(a)))
            }
            ...
            "mathbb" => {
                ...
                if let MathNode::Sym(ref s) = inner {
                    let converted: String =
                        s.chars().map(|c| mathbb_char(c).unwrap_or(c)).collect();
            ...
            _ => {
                if let Some(s) = latex_sym(&cmd) {
                    if s.is_empty() {
                        Some(MathNode::Space(0.0))
                    } else {
                        Some(MathNode::Sym(s.into()))
                    }
                } else {
    ...
    fn left_right(&mut self) -> MathNode {
        ...
        while self.i < self.ch.len() {
            if self.ch[self.i] == '\\' && self.i + 1 < self.ch.len() {
                let cs = self.i + 1;
                let mut ce = cs;
                while ce < self.ch.len() && self.ch[ce].is_ascii_alphabetic() {
                    ce += 1;
                }
...
fn parse_latex(input: &str) -> MathNode {
    P::new(input).top()
}
...
fn meas(n: &MathNode, ui: &Ui, sz: f32) -> Met {
    match n {
        ...
        MathNode::Sup(b, s) => {
            let bm = meas(b, ui, sz);
            let sm = meas(s, ui, sz * SR);
            Met {
                w: bm.w + sm.w + 1.0,
                asc: bm.asc.max(bm.asc * 0.5 + sm.asc),
                desc: bm.desc,
            }
        }
        MathNode::Sub(b, s) => {
            ...
            Met {
                w: bm.w + sm.w + 1.0,
                asc: bm.asc,
                desc: bm.desc.max(sd + sm.desc),
            }
        }
        MathNode::SubSup(b, sub, sup) => {
            let bm = meas(b, ui, sz);
            let ssub = meas(sub, ui, sz * SR);
            let ssup = meas(sup, ui, sz * SR);
            ...
            Met {
                w: bm.w + sw + 1.0,
                asc: a,
                desc: bm.desc.max(sd + ssub.desc),
            }
        }
        MathNode::Frac(nu, de) => {
            let nm = meas(nu, ui, sz * 0.85);
            let dm = meas(de, ui, sz * 0.85);
...
fn draw(n: &MathNode, p: &egui::Painter, x: f32, bl: f32, sz: f32, col: Color32, ui: &Ui) -> f32 {
    match n {
        ...
        MathNode::Frac(nu, de) => {
            let fs = sz * 0.85;
            let nm = meas(nu, ui, fs);
            let dm = meas(de, ui, fs);
            ...
            p.line_segment(
                [Pos2::new(x + 1.0, line_y), Pos2::new(x + tw - 1.0, line_y)],
        ...
        MathNode::Matrix { rows, left, right } => {
            ...
            let th: f32 =
                rm.iter().map(|m| m.h()).sum::<f32>() + (rows.len() as f32 - 1.0).max(0.0) * MR;
            ...
            if has_d {
                ...
                if *left != ' ' {
                    draw_bracket(p, *left, x + 2.0, top, bot, s, true);
                }
                if *right != ' ' {
                    draw_bracket(p, *right, cx0 + tw + 2.0, top, bot, s, false);
                }
            }
            tw + dw * 2.0 + 8.0
        }
        MathNode::Delimited(inner, l, r) => {
            ...
            let s = egui::Stroke::new(1.5, col);
            if *l != ' ' {
                draw_bracket(p, *l, x, top, bot, s, true);
            }
            draw(inner, p, x + bw, bl, sz, col, ui);
            if *r != ' ' {
                draw_bracket(p, *r, x + bw + im.w, top, bot, s, false);
            }
            im.w + bw * 2.0
        }
        MathNode::Accent(inner, kind) => {
            ...
            match kind {
                ...
                AccentKind::Hat => {
                    ...
                    p.line_segment(
                        [Pos2::new(cx - hw, ay + 2.0), Pos2::new(cx, ay - 2.0)],
                        stroke,
                    );
                    p.line_segment(
                        [Pos2::new(cx, ay - 2.0), Pos2::new(cx + hw, ay + 2.0)],
                        stroke,
                    );
                }
                AccentKind::Bar => {
                ...
                AccentKind::Vec => {
                    ...
                    p.line_segment([Pos2::new(cx - hw, ay), Pos2::new(cx + hw, ay)], stroke);
                    p.line_segment(
                        [Pos2::new(cx + hw - 3.0, ay - 2.5), Pos2::new(cx + hw, ay)],
                        stroke,
                    );
                    p.line_segment(
                        [Pos2::new(cx + hw - 3.0, ay + 2.5), Pos2::new(cx + hw, ay)],
        ...
        MathNode::Underbrace(content, label) => {
            ...
            p.line_segment(
                [Pos2::new(x + 2.0, brace_y), Pos2::new(mid_x, tip_y)],
                stroke,
            );
            p.line_segment(
                [Pos2::new(mid_x, tip_y), Pos2::new(x + cw - 2.0, brace_y)],
                stroke,
            );
            // Draw label below
            let lsz = sz * 0.7;
...
fn draw_bracket(
    ...
    match ch {
        ...
        '|' => {
            p.line_segment(
                [Pos2::new(x + bw / 2.0, top), Pos2::new(x + bw / 2.0, bot)],
                s,
            );
        }
        '‖' => {
            p.line_segment(
                [Pos2::new(x + bw * 0.3, top), Pos2::new(x + bw * 0.3, bot)],
                s,
            );
            p.line_segment(
                [Pos2::new(x + bw * 0.7, top), Pos2::new(x + bw * 0.7, bot)],
                s,
            );
        }
        _ => {}
...
pub fn render_math_equation(ui: &mut Ui, latex: &str, display: bool, text_color: Color32) {
    ...
    if display {
        ...
        let (rect, _) =
            ui.allocate_exact_size(Vec2::new(avail, m.h() + 12.0), egui::Sense::hover());
        let painter = ui.painter_at(rect);
        painter.rect_filled(
            rect,
            6.0,
            Color32::from_rgba_premultiplied(88, 166, 255, 10),
        );
        painter.rect_stroke(
            rect,
            6.0,
            egui::Stroke::new(0.5, Color32::from_rgb(200, 215, 230)),
            egui::StrokeKind::Outside,
        );
        ...
        let (rect, _) =
            ui.allocate_exact_size(Vec2::new(m.w + 4.0, m.h() + 2.0), egui::Sense::hover());
        let painter = ui.painter_at(rect);
